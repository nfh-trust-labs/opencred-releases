...
var _jsonldNs__namespace = /*#__PURE__*/_interopNamespace(_jsonldNs);
var jose2__namespace = /*#__PURE__*/_interopNamespace(jose2);
...
async function verifyProof(credential, options) {
  try {
    ...
    const verifier = crypto.createVerify(signAlg);
...
async function verifyEdDsaProof(credential, options) {
  try {
    ...
    const verified = crypto.verify(null, dataToVerify, publicKey, signatureBytes);
    return verified ? { verified: true } : { verified: false, error: "Signature verification failed" };
  } catch (error) {
    if (error instanceof TypeError || error instanceof ReferenceError || error instanceof SyntaxError || error instanceof RangeError) {
      throw error;
...
async function verifyJwsProof(jwsString, didResolver) {
  ...
  let publicKey;
  try {
    publicKey = await jose2.importJWK(vm.publicKeyJwk, alg);
  } catch {
    return { name: "signature", passed: false, detail: "Failed to import public key from JWK" };
  }
  try {
    await jose2.compactVerify(jwsString, publicKey, {
...
async function verifyVcJwt(jwt, didResolver) {
  try {
    assertJwtSize(jwt);
    const header = jose2__namespace.decodeProtectedHeader(jwt);
    if (!header.alg) {
      return {
    ...
    const payload = jose2__namespace.decodeJwt(jwt);
    if (!payload.iss) {
      return {
    ...
    await jose2__namespace.jwtVerify(jwt, publicKey, {
...
async function verifySdJwtVc(sdJwtVc, didResolver, options) {
  try {
    ...
    const header = jose2__namespace.decodeProtectedHeader(issuerJwt);
    if (!header.alg) {
      return {
    ...
    const payload = jose2__namespace.decodeJwt(issuerJwt);
    if (!payload.iss) {
      return {
    ...
    await jose2__namespace.jwtVerify(issuerJwt, publicKey, {
...
async function verifyKeyBindingJwt(keyBindingJwt, issuerPayload, fullSdJwtVc, options) {
  try {
    ...
    const kbHeader = jose2__namespace.decodeProtectedHeader(keyBindingJwt);
    ...
    const holderKey = await jose2__namespace.importJWK(cnf.jwk, kbHeader.alg);
    const { payload: kbPayload } = await jose2__namespace.jwtVerify(keyBindingJwt, holderKey, {
      algorithms: ALLOWED_ALGORITHMS2
    });
...
function computeSdHash(sdJwtWithoutKb, algorithm) {
  ...
  const hash = crypto.createHash(nodeAlg).update(sdJwtWithoutKb, "ascii").digest();
...
async function resolveAndValidateIp(hostname) {
  ...
  let family = 4;
  try {
    addresses = await promises.resolve4(hostname);
  } catch {
  }
  if (addresses.length === 0) {
    try {
      addresses = await promises.resolve6(hostname);
      family = 6;
    } catch {
...
async function extractEmbeddedCredential(pdfBytes) {
  ...
  if (!infoRef)
    return { kind: "missing" };
  const infoDict = doc.context.lookup(infoRef, pdfLib.PDFDict);
  if (!infoDict)
    return { kind: "missing" };
  const raw = infoDict.lookup(pdfLib.PDFName.of(PDF_CREDENTIAL_INFO_KEY));
  if (!raw)
    return { kind: "missing" };
  if (raw instanceof pdfLib.PDFString)
    return { kind: "present", value: raw.asString() };
  if (raw instanceof pdfLib.PDFHexString)
